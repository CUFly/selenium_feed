...
def save2csv(dic_list, n):
    filename = "0319ReportType/0319ReportType%d.csv" % n
    print("创建文件"+filename)
...
buttun = browser.find_element_by_xpath("//div[@class='ant-tabs-nav ant-tabs-nav-animated']/div/div[5]")
buttun.click()
time.sleep(1)
...
for i in range(1, 12):
    ...
    # 进入页面，定位信息
    # time.sleep(3)
    #                                         //*[@id="root"]/div/div[2]/div/div[2]/div/div[1]/div[3]/div[5]/span[1]
    Type_btn = browser.find_element_by_xpath("//*[@id='root']/div/div[2]/div/div[2]/div/div[1]/div[3]/div[5]/span[%d]" % i)
    time.sleep(1)
    browser.execute_script("arguments[0].click();", Type_btn)
    time.sleep(1)

    Type = browser.find_element_by_xpath("//*[@id='root']/div/div[2]/div/div[2]/div/div[1]/div[3]/div[5]/span[%d]" % i).text
    # time.sleep(3)
