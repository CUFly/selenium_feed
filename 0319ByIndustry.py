...
def save2csv(dic_list, n):
    filename = "0319Industry/0319ByIndustry%d.csv" % n
    print("创建文件"+filename)
...
buttun = browser.find_element_by_xpath("//div[@class='ant-tabs-nav ant-tabs-nav-animated']/div/div[3]")
buttun.click()
...
for i in range(1, 115):
    ...
    # 进入页面，定位信息
    # time.sleep(3)
    industry_btn = browser.find_element_by_xpath("//*[@id='root']/div/div[2]/div/div[2]/div/div[1]/div[3]/div[3]/span[%d]" % i)
    time.sleep(1)
    browser.execute_script("arguments[0].click();", industry_btn)
    time.sleep(1)

    industry = browser.find_element_by_xpath("//*[@id='root']/div/div[2]/div/div[2]/div/div[1]/div[3]/div[3]/span[%d]" % i).text
    # time.sleep(3)
