...
def save2csv(dic_list):
    pd.DataFrame(dic_list).to_csv('0319ByYear5.csv', encoding='utf-8-sig')
...
apt_buttun = browser.find_element_by_xpath("//div[@class='ant-tabs-nav ant-tabs-nav-animated']/div/div[1]")
apt_buttun.click()
...
for i in range(10, 16):
    # 进入页面，定位信息
    # time.sleep(3)
    year_btn = browser.find_element_by_xpath("//*[@id='root']/div/div[2]/div/div[2]/div/div[1]/div[3]/div[1]/span[%d]" % i)
    browser.execute_script("arguments[0].click();", year_btn)
    time.sleep(1)

    year = browser.find_element_by_xpath("//*[@id='root']/div/div[2]/div/div[2]/div/div[1]/div[3]/div[1]/span[%d]" % i).text
    # time.sleep(3)
